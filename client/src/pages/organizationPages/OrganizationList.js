...
export default function OrganizationList() {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this organization?")) {
            await deleteData("organizations", id); 
                        ...
                        <tbody>
                            {paginatedOrganizations.map(org => (
                                <tr key={org.id}>
                                    ...
                                    <td>
                                        <Link className="btn btn-edit btn-warning btn-sm me-2 btn-edit" to={`/organizations/edit/${org.id}`}>
                                            Edit
                                        </Link>
                                        <button className="btn btn-delete btn-danger btn-sm" onClick={() => handleDelete(org.id)}>
                                            Delete
                                        </button>
                                    </td>
                                </tr>
